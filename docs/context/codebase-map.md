...
**V0.1 已实施（2026-08-28 验收通过）**。设计权威在前，实现路径在后。
...
| V0.1 合同/验收   | `docs/architecture/v0.1-standalone-contract.md`, `docs/testing/v0.1-acceptance-checklist.md` | 契约与 25 项验收清单（已 executed，accepted） | 2026-08-28 | high |
...
| 验证工具链      | `tools/{verify-docs,resolve-bom,verify-v0.1,verify-v0.1-remote}.sh` | 静态/BOM/集成 25 项/远端验收 | 2026-08-28 | high |
...
| 修订安全/脱敏设计    | 技术架构 §12 | 项目架构 §11.1 三道防线、collector redaction processors | canary 三路 absence（`verify-v0.1.sh` E 段） | 2026-08-28 | high |
...
| `tools/verify-v0.1.sh` | 25 项验收逻辑 + 时序轮询（Tempo 索引延迟/首导出窗口） | 改动后 `bash -n` + 远端完整重跑 |
